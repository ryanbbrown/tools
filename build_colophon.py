...
def format_commit_message(message):
    ...
    linkified = re.sub(
        issue_pattern,
        r'<a href="https://github.com/ryanbbrown/tools/issues/\1">#\1</a>',
        linkified,
    )
...
"""
    ...
    html_content += f"""
    <p>This page lists {tool_count} tools and their development history, including commit messages that may link to LLM transcripts used to produce the code.</p>
"""

    # Modified tool heading HTML
    for page_name, page_data in sorted_pages:
        tool_url = f"/{page_name.replace('.html', '')}"
        github_url = f"https://github.com/ryanbbrown/tools/blob/main/{page_name}"
        commits = page_data.get("commits", [])
...
"""
        ...
        for commit in commits:
            ...
            commit_url = f"https://github.com/ryanbbrown/tools/commit/{commit_hash}"
